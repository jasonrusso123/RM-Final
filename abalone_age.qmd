...
sigma2_hat <- pmax(fitted(var_fit), 1e-6)  # avoid zeros
w <- 1 / sigma2_hat                        # weights ∝ 1/Var(e_i)
...
cv_enet <- cv.glmnet(x_train, y_train, 
                     alpha = 0.5,        # your ENet tuning choice
                     family = "gaussian")
...
# Combine into a single data frame for easier plotting
df_pred <- data.frame(
  Actual = y_test,
...
# Predict on test set 
# glmnet Poisson returns log link predictions by default
...
mse_ols <- mean((y_test - ols_pred)^2)
mse_enet <- mean((y_test - pred)^2)
mse_ns <- mean((y_test - pred_ns)^2)
mse_pois <- mean((y_test - pois_pred)^2)

results <- data.frame(
  Model = c("OLS", "Elastic Net", "Natural Splines", "Poisson Elastic Net", "Decision Tree"),
  MSE = c(mse_ols, mse_enet, mse_ns, mse_pois, mse_tree),
  RMSE = c(rmse_ols, rmse, rmse_ns, rmse_pois, rmse_tree),
  MAE = c(mae_ols, mae, mae_ns, mae_pois, NA)
)

results$MAE[5] <- mean(abs(y_test - pred_tree))
...
```{r}
eig <- eigen(cov_mat)$values   # extract only the numeric eigenvalues
